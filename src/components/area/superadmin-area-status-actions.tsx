...
interface SuperadminAreaStatusActionsProps {
  area: Area;
  enumeratorId: string;
}
...
export function SuperadminAreaStatusActions({
  area,
  enumeratorId,
}: SuperadminAreaStatusActionsProps) {
  ...
  const { mutate: requestCompletion, isLoading: isRequestingCompletion } =
    api.superadmin.requestCompletion.useMutation({
      onSuccess: () => {
  ...
  } = api.superadmin.requestRevisionCompletion.useMutation({
    onSuccess: () => {
      utils.enumerator.getAssignedArea.invalidate();
    },
  });
  ...
  const { mutate: requestWithdrawal, isLoading: isRequestingWithdrawal } =
    api.superadmin.requestWithdrawal.useMutation({
      onSuccess: () => {
        utils.enumerator.getAssignedArea.invalidate();
      },
    });
  ...
  const confirmAction = () => {
    switch (actionType) {
      case "completion":
        requestCompletion({ enumeratorId });
        break;
      case "revision":
        requestRevisionCompletion({ enumeratorId });
        break;
      case "withdrawal":
        requestWithdrawal({ enumeratorId });
        break;
    }
          ...
          <AlertDialogContent className="sm:max-w-[425px] z-[1000]">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="pt-2">
                {actionType === "completion" &&
                  "Are you sure you want to request completion of this area? This action indicates that the enumerator has completed all the required surveys in this area."}
                {actionType === "revision" &&
                  "Are you sure you want to submit this area for revision completion? This indicates the enumerator has addressed all revision requests."}
                {actionType === "withdrawal" &&
                  "Are you sure you want to request withdrawal from this area? This will notify administrators that the enumerator can no longer continue surveying this area."}
              </AlertDialogDescription>
            </AlertDialogHeader>
