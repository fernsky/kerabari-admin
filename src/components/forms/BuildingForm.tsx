...
const BuildingForm = ({ areaId }: BuildingFormProps) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      const tableColumns = [
        ...
        { header: "Businesses", dataKey: "businesses" },
      ...
      doc.save(`building_survey_${areaDetails.code}.pdf`);
    } catch (error) {
  ...
  return (
    <PrintButton
      icon={Printer}
      label={`Building Form (${tokenCount})`}
      onPrint={generatePDF}
